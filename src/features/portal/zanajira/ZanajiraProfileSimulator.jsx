...
const CHECKLIST = [
  ...
  { key: "certificates", label: "Nime-upload Certificates za PDF (chini ya 2MB)" },
  { key: "computer",     label: "Nimejaza Ujuzi wa Kompyuta (Computer Literacy)" },
  { key: "referees",     label: "Nimeweka Watoa Mapendekezo (Referees)" },
  { key: "declaration",  label: "Nimefanya Tamko la Ukweli (Declaration)" },
];

export default function ZanajiraProfileSimulator({ setProfileScore }) {
  const [profile, setProfile] = useState({
    personal: false, academic: false, certificates: false,
    computer: false, referees: false, declaration: false,
  });
  ...
  const toggle = (field) => setProfile((prev) => ({ ...prev, [field]: !prev[field] }));
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <h2 className="text-xl sm:text-2xl font-extrabold text-slate-900">
          Ukamilishaji wa ZanAjira Profile
        </h2>
        <p className="text-slate-500 mt-1 text-sm">
          Weka alama kwenye kila sehemu uliyokwisha ijaza kwenye portal ya ZanAjira.
        </p>
      </div>
