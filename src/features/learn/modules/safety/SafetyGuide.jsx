...
const SafetyGuide = () => {
  const [step, setStep] = useState(() => {
    const saved = sessionStorage.getItem("safetyGuideStep");
    return saved ? parseInt(saved, 10) : 1;
  });
  const [lang, setLang] = useState(() => {
    return sessionStorage.getItem("safetyGuideLang") || "sw";
  });
  ...
  useEffect(() => { sessionStorage.setItem("safetyGuideStep", step); }, [step]);
  useEffect(() => { sessionStorage.setItem("safetyGuideLang", lang); }, [lang]);
  useEffect(() => { window.scrollTo({ top: 0, behavior: "instant" }); }, [step]);

  const handleBackToLearn = () => {
    sessionStorage.removeItem("safetyGuideStep");
    sessionStorage.removeItem("safetyGuideLang");
    navigate("/learn");
  };
  ...
  return (
    <div className="w-full max-w-4xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5 sm:mb-6">
          <div className="flex items-center gap-4">
            ...
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">
              {isSw ? `Sehemu ${step - 1} / ${totalSteps - 1}` : `Part ${step - 1} / ${totalSteps - 1}`}
            </span>
          </div>
          <button
      ...
      <div className={`w-full transition-all duration-300 ${
        step === 1 ? "" : "bg-white rounded-2xl sm:rounded-3xl border border-slate-200 shadow-sm overflow-hidden"
      }`}>
        <div className={step === 1 ? "p-0" : "p-5 sm:p-7 lg:p-10"}>
          <SafetyContent step={step} setStep={setStep} lang={lang} setLang={setLang} />
        </div>
          ...
          <div className="px-5 sm:px-7 lg:px-10 py-4 sm:py-5 bg-slate-50 border-t border-slate-100">
            <div className="flex items-center justify-between gap-3">
              ...
              {step < totalSteps && (
                <button
                  onClick={() => setStep(step + 1)}
                  className="px-6 sm:px-8 py-3 bg-teal-600 hover:bg-teal-700 text-white rounded-xl font-bold text-sm transition cursor-pointer active:scale-[0.98] shadow-sm"
                >
                  {isSw ? "Endelea →" : "Next →"}
                </button>
              )}
